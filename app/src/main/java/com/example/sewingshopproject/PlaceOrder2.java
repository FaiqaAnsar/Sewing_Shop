...
public class PlaceOrder2 extends AppCompatActivity {
    ...
    private void submit() {
        ...
        if (TextUtils.isEmpty(username)) {
            ...
            reff.child("Tailor2").child(id).setValue(customer).addOnCompleteListener(new OnCompleteListener<Void>() {
                @Override
                public void onComplete(@NonNull Task<Void> task) {
                    if (task.isSuccessful()) {
                        Toast.makeText(PlaceOrder2.this, "User Details inserted", Toast.LENGTH_SHORT).show();
