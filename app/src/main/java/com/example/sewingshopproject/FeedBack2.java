...
public class FeedBack2 extends AppCompatActivity {
    ...
    private void submit() {
        ...
        if (TextUtils.isEmpty(username)) {
            ...
            reff.child("feedb2").child(id).setValue(feed).addOnCompleteListener(new OnCompleteListener<Void>() {
                @Override
                public void onComplete(@NonNull Task<Void> task) {
                    if (task.isSuccessful()) {
                        Toast.makeText(FeedBack2.this, "User Details inserted", Toast.LENGTH_SHORT).show();
                    }
                }
            });
